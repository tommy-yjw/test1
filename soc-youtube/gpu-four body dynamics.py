...
with tqdm(total=2, desc="Solving ODEs") as pbar:
    ...
    pbar.update(1)
    x_sol_2 = odeint(dynamics2, x0.get(), t.get(), args=(laplacian_matrix.get(), result_1.get(), lambda_coeff))
    pbar.update(1)
    # x_sol_3 = odeint(dynamics3, x0.get(), t.get(), args=(laplacian_matrix.get(), result_2.get(), lambda_coeff, result_3.get()))
    # pbar.update(1)
...
plt.plot(cp.linspace(0, t_final, len(x_sol_1)).get(), x_sol_1[:, 0], label=f'Higher-order coeff_2=0,Higher-order coeff_3=0',
         color='blue')
...
plt.xlabel('Time')
plt.ylabel('x')
plt.title('soc-youtube-four body dynamics')
plt.legend()
plt.show()
