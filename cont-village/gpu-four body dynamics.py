...
def dynamics2(x, t, laplacian_matrix, result_1, lambda_coeff):
    ...
    return -0.3 * laplacian_matrix.dot(x) + (1 - lambda_coeff) * update_result_1(x, result_1)


def dynamics3(x, t, laplacian_matrix, result_2, lambda_coeff, result_3):
    # laplacian_matrix = cp.array(laplacian_matrix)
...
with tqdm(total=3, desc="Solving ODEs") as pbar:
    x_sol_1 = odeint(dynamics1, x0.get(), t.get(), args=(laplacian_matrix.get(),))
    pbar.update(1)
    x_sol_2 = odeint(dynamics2, x0.get(), t.get(),
                     args=(laplacian_matrix.get(), result_1.get(), lambda_coeff,))
    pbar.update(1)
    x_sol_3 = odeint(dynamics3, x0.get(), t.get(),
                     args=(laplacian_matrix.get(), result_2.get(), lambda_coeff, result_3.get(), ))
    pbar.update(1)
...
plt.xlabel('Time')
plt.ylabel('x')
plt.title('cont village-four body dynamics')
plt.legend()
plt.show()
print("Higher-order coeff_2=0,Higher-order coeff_3=0:\n", x_sol_1[t_end][0])
print("Higher-order coeff_2={1 - lambda_coeff},Higher-order coeff_3=0:\n", x_sol_2[t_end_2][0])
print("Higher-order coeff={lambda_coeff_1},Higher-order coeff_3={lambda_coeff_2}:\n", x_sol_3[t_end_3][0])
