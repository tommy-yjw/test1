...
def dynamics2(x, t, laplacian_matrix,u0):
    return -0.9 * laplacian_matrix.dot(x) + 0.1 * (u0 - x)

def dynamics3(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(1)):
        if i == 0:
            final_result += 0.8 * update_result_3(x, df, node_index, result_1[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result + 0.1 * (u0 - x)

def dynamics4(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0):
    final_result = np.zeros(num_nodes_1, dtype='float32')
    result_2 = {}
    for i in range(2):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(2)):
        if i == 0:
            ...
            final_result += 0.7 * update_result_4(x, df, node_index, result_2[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result + 0.1 * (u0 - x)

def dynamics5(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0):
    final_result = np.zeros(num_nodes_1, dtype='float32')
    result_3 = {}
    for i in range(3):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(3)):
        if i == 0:
            ...
            final_result += 0.5 * update_result_5(x, df, node_index, result_3[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result + 0.1 * (u0 - x)
def dynamics6(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0):
    final_result = np.zeros(num_nodes_1, dtype='float32')
    result_4 = {}
    for i in range(4):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(4)):
        if i == 0:
            ...
            final_result += 0.4 * update_result_6(x, df, node_index, result_4[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result + 0.1 * (u0 - x)
def dynamics7(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0):
    final_result = np.zeros(num_nodes_1, dtype='float32')
    result_5 = {}
    for i in range(5):
...
# 定义动力学方程
def dynamics8(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0):
    final_result = np.zeros(num_nodes_1, dtype='float32')
    result_6 = {}
    for i in range(6):
...
u0 = np.random.rand(num_nodes_1)
# 系数之和为1
lambda_coeffs = np.array([0.1, 0.2, 0.1, 0.1, 0.2, 0.1])

# 求解ODE
with tqdm(total=7, desc="Solving ODE") as pbar:
    x_sol_2 = odeint(dynamics2, x0, t, args=(laplacian_matrix, u0))
    pbar.update(1)
    x_sol_3 = odeint(dynamics3, x0, t, args=(laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0))
    pbar.update(1)
    x_sol_4 = odeint(dynamics4, x0, t, args=(laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0))
    pbar.update(1)
    x_sol_5 = odeint(dynamics5, x0, t, args=(laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0))
    pbar.update(1)
    x_sol_6 = odeint(dynamics6, x0, t, args=(laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0))
    pbar.update(1)
    x_sol_7 = odeint(dynamics7, x0, t, args=(laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0))
    pbar.update(1)
    x_sol_8 = odeint(dynamics8, x0, t, args=(laplacian_matrix, df_list, node_index_list, lambda_coeffs, u0))
    pbar.update(1)
...
plt.xlabel('Time')
plt.ylabel('x')
plt.title('cont village Multi-body dynamics with stubbornness')
plt.legend()
plt.show()
max_value = np.max(x_sol_2[t_final])
min_value = np.min(x_sol_2[t_final])
difference = max_value - min_value
print(f"two-body dynamics difference:\n",  difference)

max_value = np.max(x_sol_3[t_final])
min_value = np.min(x_sol_3[t_final])
difference = max_value - min_value
print(f"three-body dynamics difference:\n",  difference)

max_value = np.max(x_sol_4[t_final])
min_value = np.min(x_sol_4[t_final])
difference = max_value - min_value
print(f"four-body dynamics difference:\n",  difference)

max_value = np.max(x_sol_5[t_final])
min_value = np.min(x_sol_5[t_final])
difference = max_value - min_value
print(f"five-body dynamics difference:\n",  difference)

max_value = np.max(x_sol_6[t_final])
min_value = np.min(x_sol_6[t_final])
difference = max_value - min_value
print(f"six-body dynamics difference:\n",  difference)

max_value = np.max(x_sol_7[t_final])
min_value = np.min(x_sol_7[t_final])
difference = max_value - min_value
print(f"seven-body dynamics difference:\n",  difference)

max_value = np.max(x_sol_8[t_final])
min_value = np.min(x_sol_8[t_final])
difference = max_value - min_value
print(f"eight-body dynamics difference:\n",  difference)
