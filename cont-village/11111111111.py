...
def update_result_3(x0, df, node_index, result):
    ...
    for _, row in df.iterrows():
        ...
        product = cp.prod(cp.array([x0[node] for node in nodes[1:]]))
...
def dynamics3(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs,result):
    final_result = cp.zeros(num_nodes_1, dtype='float32')
    result = {}
    for i in range(1):
        result[i] = cp.zeros(num_nodes_1, dtype='float32')
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(1)):
        if i == 0:
            final_result += 0.9 * update_result_3(x, df, node_index, result[i])
...
def dynamics4(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(2)):
        if i == 0:
            final_result += lambda_coeffs[i] * update_result_3(x, df, node_index, result[i].get()).get()
        elif i == 1:
            final_result += 0.8 * update_result_4(x, df, node_index, result[i].get()).get()
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
...
def dynamics5(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(3)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_4(x, df, node_index, result[i].get()).get()
        elif i == 2:
            final_result += 0.6 * update_result_5(x, df, node_index, result[i].get()).get()
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
def dynamics6(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(4)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_5(x, df, node_index, result[i].get()).get()
        elif i == 3:
            final_result += 0.45 * update_result_6(x, df, node_index, result[i].get()).get()
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
def dynamics7(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(5)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_6(x, df, node_index, result[i].get()).get()
        elif i == 4:
            final_result += 0.3 * update_result_7(x, df, node_index, result[i].get()).get()
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
...
def dynamics8(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(6)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_7(x, df, node_index, result[i].get()).get()
        elif i == 5:
            final_result += lambda_coeffs[i] * update_result_8(x, df, node_index, result[i].get()).get()
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
...
x0 = cp.random.rand(num_nodes_1)
# 系数之和为1
lambda_coeffs = cp.array([0.1, 0.2, 0.15, 0.15, 0.2, 0.1])

# 求解ODE
with tqdm(total=7, desc="Solving ODE") as pbar:
    x_sol_2 = odeint(dynamics2, x0.get(), t.get(), args=(laplacian_matrix.get(),))
    pbar.update(1)
    x_sol_3 = odeint(dynamics3, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs.get()))
    pbar.update(1)
    x_sol_4 = odeint(dynamics4, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs.get()))
    pbar.update(1)
    x_sol_5 = odeint(dynamics5, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs.get()))
    pbar.update(1)
    x_sol_6 = odeint(dynamics6, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs.get()))
    pbar.update(1)
    x_sol_7 = odeint(dynamics7, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs.get()))
    pbar.update(1)
    x_sol_8 = odeint(dynamics8, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs.get()))
    pbar.update(1)
...
for idx, x_sol in enumerate([x_sol_2, x_sol_3, x_sol_4, x_sol_5, x_sol_6, x_sol_7, x_sol_8]):
    for i in tqdm(range(1, len(t)), desc=f"Checking convergence for x_sol_{idx+2}"):
        for j in range(num_nodes_1):
            if cp.abs(x_sol[i][j] - x_sol[i - 1][j]) >= tolerance:
                break
...
plt.plot(cp.linspace(0, t, len(x_sol_2)).get(), x_sol_2[:, 0], color='yellow', label='x_sol_2')
plt.plot(cp.linspace(0, t, len(x_sol_3)).get(), x_sol_3[:, 0], color='orange', label='x_sol_3')
plt.plot(cp.linspace(0, t, len(x_sol_4)).get(), x_sol_4[:, 0], color='blue', label='x_sol_4')
plt.plot(cp.linspace(0, t, len(x_sol_5)).get(), x_sol_5[:, 0], color='green', label='x_sol_5')
plt.plot(cp.linspace(0, t, len(x_sol_6)).get(), x_sol_6[:, 0], color='brown', label='x_sol_6')
plt.plot(cp.linspace(0, t, len(x_sol_7)).get(), x_sol_7[:, 0], color='cyan', label='x_sol_7')
plt.plot(cp.linspace(0, t, len(x_sol_8)).get(), x_sol_8[:, 0], color='olive', label='x_sol_8')
for j in range(num_nodes_1):
    plt.plot(cp.linspace(0, t, len(x_sol_2)).get(), x_sol_2[:, j], color='yellow', label='x_sol_2')
    plt.plot(cp.linspace(0, t, len(x_sol_3)).get(), x_sol_3[:, j], color='orange', label='x_sol_3')
    plt.plot(cp.linspace(0, t, len(x_sol_4)).get(), x_sol_4[:, j], color='blue', label='x_sol_4')
    plt.plot(cp.linspace(0, t, len(x_sol_5)).get(), x_sol_5[:, j], color='green', label='x_sol_5')
    plt.plot(cp.linspace(0, t, len(x_sol_6)).get(), x_sol_6[:, j], color='brown', label='x_sol_6')
    plt.plot(cp.linspace(0, t, len(x_sol_7)).get(), x_sol_7[:, j], color='cyan', label='x_sol_7')
    plt.plot(cp.linspace(0, t, len(x_sol_8)).get(), x_sol_8[:, j], color='olive', label='x_sol_8')
plt.xlabel('Time')
plt.ylabel('x')
