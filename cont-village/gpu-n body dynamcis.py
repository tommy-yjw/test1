...
def dynamics4(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(2)):
        if i == 0:
            final_result += lambda_coeffs[i] * update_result_3(x, df, node_index, result[i])
        elif i == 1:
            final_result += 0.8 * update_result_4(x, df, node_index, result[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
...
def dynamics5(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(3)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_4(x, df, node_index, result[i])
        elif i == 2:
            final_result += 0.6 * update_result_5(x, df, node_index, result[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
def dynamics6(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(4)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_5(x, df, node_index, result[i])
        elif i == 3:
            final_result += 0.45 * update_result_6(x, df, node_index, result[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
def dynamics7(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(5)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_6(x, df, node_index, result[i])
        elif i == 4:
            final_result += 0.3 * update_result_7(x, df, node_index, result[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
...
def dynamics8(x, t, laplacian_matrix, df_list, node_index_list, lambda_coeffs):
    ...
    for df, node_index, lambda_coeff, i in zip(df_list, node_index_list, lambda_coeffs, range(6)):
        if i == 0:
            ...
            final_result += lambda_coeffs[i] * update_result_7(x, df, node_index, result[i])
        elif i == 5:
            final_result += lambda_coeffs[i] * update_result_8(x, df, node_index, result[i])
    return -0.1 * laplacian_matrix.dot(x) + final_result.get()
...
with tqdm(total=7, desc="Solving ODE") as pbar:
    ...
    pbar.update(1)
    x_sol_4 = odeint(dynamics4, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs))
    pbar.update(1)
    x_sol_5 = odeint(dynamics5, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs))
    pbar.update(1)
    x_sol_6 = odeint(dynamics6, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs))
    pbar.update(1)
    x_sol_7 = odeint(dynamics7, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs))
    pbar.update(1)
    x_sol_8 = odeint(dynamics8, x0.get(), t.get(), args=(laplacian_matrix.get(), df_list, node_index_list, lambda_coeffs))
    pbar.update(1)
...
for j in range(num_nodes_1):
    plt.plot(cp.linspace(0, t, len(x_sol_2)).get(), x_sol_2[:, j], color='yellow')
    plt.plot(cp.linspace(0, t, len(x_sol_3)).get(), x_sol_3[:, j], color='orange')
    plt.plot(cp.linspace(0, t, len(x_sol_4)).get(), x_sol_4[:, j], color='blue')
    plt.plot(cp.linspace(0, t, len(x_sol_5)).get(), x_sol_5[:, j], color='green')
    plt.plot(cp.linspace(0, t, len(x_sol_6)).get(), x_sol_6[:, j], color='brown')
    plt.plot(cp.linspace(0, t, len(x_sol_7)).get(), x_sol_7[:, j], color='cyan')
    plt.plot(cp.linspace(0, t, len(x_sol_8)).get(), x_sol_8[:, j], color='olive')
plt.xlabel('Time')
plt.ylabel('x')
plt.title('cont village Multi-body dynamics')
plt.legend()
plt.show()
