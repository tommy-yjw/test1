...
def update_result_1(x0):
    ...
    for _, row in df2.iterrows():
        ...
        result_1[node1] += x0[node2] * x0[node3] - x0[node1] ** 2
        result_1[node2] += x0[node1] * x0[node3] - x0[node2] ** 2
        result_1[node3] += x0[node1] * x0[node2] - x0[node3] ** 2
...
# lambda_coeff_1 = 0.7 * (1 - lambda_coeff)
#
# # 确保lambda_coeff_1和lambda_coeff_2系数之和为lambda_coeff
#
# lambda_coeff_2 = 0.3 * (1 - lambda_coeff)
...
def dynamics2(x, t, laplacian_matrix, result_1, lambda_coeff):
    update_result_1(x)
    return -lambda_coeff * laplacian_matrix.dot(x) + (1 - lambda_coeff) * result_1


# def dynamics3(x, t, laplacian_matrix, result_1, lambda_coeff, result):
#     update_result_1(x)
#     update_result(x)
#     return -lambda_coeff * laplacian_matrix.dot(x) + lambda_coeff_1 * result_1 + lambda_coeff_2 * result
...
with tqdm(total=2, desc="Solving ODEs") as pbar:
    ...
    pbar.update(1)
    # x_sol_3 = odeint(dynamics3, x0, t, args=(laplacian_matrix, result_1, lambda_coeff, result))
    # pbar.update(1)
...
plt.xlabel('Time')
plt.ylabel('x')
plt.title('coauth dblp-three body dynamics')
plt.legend()
plt.show()
